...
include "cpanel-right.php";
$id = intval($_GET['id']);

$page_info = mysql_query("select * from pages_table where id=".$id);
$page = mysql_fetch_assoc($page_info);
?>
<!DOCTYPE html>
<html lang="en">
 <head>
...
<div class="website_info_saved pageadded" style="   position: fixed;
    ...
    display:none;"><h2
        style=" color: white;
    margin: 0;
    font-size: 17pt; font-family:DroidKufi;">تم تحديث الصفحة بنجاح</h2></div>
<div class="website_info_saved complete-data" style="   position: fixed;
 ...
 <div class="head">
    <h4>تحديث صفحة</h4>
      ...
      <input type="text" placeholder="اسم الصفحة" class="page-name" value="<?php echo $page['page_name']; ?>"/>
      <textarea class='page-content' placeholder="محتوي الصفحة نص/شفرة" ><?php echo $page['page_content']; ?></textarea>
      <input type="hidden" value="<?php echo $page['id']; ?>" class="page-id"/>
     <button class="add-page">تحديث</button>
      </div>
...
$(".add-page").click(function () {
 ...
 var pageid = $(".page-id").val();
  ...
  if (pagename.length !== 0 && pagecontent.length !== 0) {
    $.ajax({
      url:'edit_page_proccess.php',
      type:'POST',
      data: {name: pagename, content: pagecontent, id: pageid},
      success: function () {
